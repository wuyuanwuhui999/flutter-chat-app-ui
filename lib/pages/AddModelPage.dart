...
class AddModelPage extends StatefulWidget {
  ...
  @override
  AddModelPageState createState() => AddModelPageState();
}

class AddModelPageState extends State<AddModelPage> {
  ...
  /// @author: wuwenqiang
  /// @description: 判断表单是否有效（所有必填项都已填写）
  /// @date: 2026-07-25
  bool _isFormValid() {
  ...
  void _onSubmit() {
    ...
    final params = {
      ...
      'companyId': _chatProvider.currentCompanyId,
    };

    addModelService(params)
        .then((res) {
      if (!mounted) {
        setState(() {
          _isLoading = false;
